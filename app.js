...
startBtn.onclick=async()=>{
 ...
 try{
  ...
  statusEl.textContent=`Scanning — ${INPUT}×${INPUT}`;resultEl.textContent="Waiting for first inference…";requestAnimationFrame(loop);
 }catch(e){statusEl.textContent="Camera permission failed";resultEl.textContent=e?.message||String(e);}
};
function stopCamera(){running=false;if(video.srcObject)video.srcObject.getTracks().forEach(t=>t.stop());video.srcObject=null;ctx.clearRect(0,0,overlay.width,overlay.height);startBtn.textContent="Start Camera";statusEl.textContent="Camera stopped";resultEl.textContent="";}
async function loop(t){if(!running)return;if(t-lastRun>=1000/FPS&&video.readyState>=2){lastRun=t;try{const detections=await detect();statusEl.textContent=`Scanning — ${INPUT}×${INPUT}`;draw(detections);}catch(e){console.error(e);statusEl.textContent="Detection error";resultEl.textContent=e?.message||String(e);running=false;startBtn.textContent="Start Camera";if(video.srcObject){video.srcObject.getTracks().forEach(t=>t.stop());video.srcObject=null;}return;}}requestAnimationFrame(loop);}
...
function decode(o,ow,oh,scale,px,py){
 ...
 if(channels!==6)throw new Error(`Loaded model output is ${JSON.stringify(s)}. V3 detector must output [1,6,8400]. Please replace best.onnx with the 9.3 MB V3 detector.`);
